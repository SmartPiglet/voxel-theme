...
trait ParameterTrait
{
    ...
    protected function getParameterMap(): array
    {
        return ['label' => function (string $value): string {
            assert($value !== '');
            $this->hasColon($value) === \false || throw new InvalidArgumentException('Label must not contain a colon.');
            return $value;
        }, 'secret' => static fn(string $value): string => mb_strtoupper(trim($value, '=')), 'algorithm' => static function (string $value): string {
            $value = mb_strtolower($value);
            in_array($value, hash_algos(), \true) || throw new InvalidArgumentException(sprintf('The "%s" digest is not supported.', $value));
            return $value;
            ...
            assert($value !== '');
            $this->hasColon($value) === \false || throw new InvalidArgumentException('Issuer must not contain a colon.');
            return $value;
        }];
    }
    /**
